...
function formatToChart(data) {
  ...
  data.forEach((item) => {
    chartData.labels.push(dayjs(item.timestamp * 1000).format("DD MMM"));
...
export default async function oneMonthData(req, res) {
  const coinUuid = req.query.uuid;
  const data = await fetch(
    `https://coinranking1.p.rapidapi.com/coin/${coinUuid}/history?referenceCurrencyUuid=${process.env.NEXT_PUBLIC_REFERENCE_CURRENCY_UUID}&timePeriod=30d`,
    options
  );
